...
from training.dataset_real_ms import ConcatDataset
#----------------------------------------------------------------------------
...
def training_loop(
    ...
    training_set2 = dnnlib.util.construct_class_by_name(**training_set_kwargs2) # subclass of training.dataset.Dataset
    ### activate datasets
    training_set.get_label(0)
    training_set1.get_label(0)
    training_set2.get_label(0)
    concat_training_set = ConcatDataset(training_set, training_set1, training_set2)
    ...
    val_set2 = dnnlib.util.construct_class_by_name(**validation_set_kwargs2) # subclass of training.dataset.Dataset
    concat_val_set = ConcatDataset(val_set, val_set1, val_set2) # subclass of training.dataset.Dataset
    ...
    latent_sampler1 = SourceSampling(dataset=training_set.data_name,
                                     label_path=training_set._path)
    df1 = latent_sampler1.get_graph()
    latent_model1 = latent_sampler1.get_causal_model()

    latent_sampler2 = SourceSampling(dataset=training_set1.data_name,
                                    label_path=training_set1._path)
    df2 = latent_sampler2.get_graph()
    latent_model2 = latent_sampler2.get_causal_model()

    latent_sampler3 = SourceSampling(dataset=training_set2.data_name,
                                    label_path=training_set2._path)
    df3 = latent_sampler3.get_graph()
    latent_model3 = latent_sampler3.get_causal_model()

    ## cov dict for the common covariance
    cov_dict = {}
    if training_set.data_name == "ukb" or training_set.data_name == "adni" or training_set.data_name == "nacc":
        cov_dict["age_max"] = max(training_set.model["age_max"], training_set1.model["age_max"], 
                                  training_set2.model["age_max"])
        cov_dict["age_min"] = min(training_set.model["age_min"], training_set1.model["age_min"], 
                                  training_set2.model["age_min"])
    ...
    if rank == 0:
        ...
        print()
        print("Dataset name 3: ", training_set2.data_name)
        print('Num images train 3: ', len(training_set2))
        print("Num images val 3: ", len(val_set2))
        print('Image shape 3: ', training_set2.image_shape)
        print('Label shape 3: ', training_set2.label_shape)
        print()
        ...
        wandb.config.update(
            {
                ...
                "dataset3": training_set2.data_name,
                "num_images3": len(training_set2),
                "image_shape3": training_set2.image_shape,
                "label_shape3": training_set2.label_shape,
                "num_images_val3": len(val_set2),
            }
        )
    ...
    if rank == 0:
        ...
        if training_set.data_name == "retinal":
            ...
            sample_labels = latent_sampler2.sampling_given_age(age=age, normalize=True).cpu().detach().numpy() ## hippocampus
            ...
            labels = np.concatenate([age_norm.reshape(-1, 1), labels[:, 1:], 
                                     sample_labels[:, 1:], sample_source3[:, 1:], 
                                     c_source.reshape(len(labels), -1)], axis=1)
            ## source 2
            ...
            age_norm = (age - cov_dict["age_min"]) / (cov_dict["age_max"] - cov_dict["age_min"])
            labels1 = np.concatenate([age_norm.reshape(-1, 1), sample_source1[:, 1:],
                                     labels1[:, 1:], sample_source3[:, 1:], 
                                     c_source.reshape(len(labels1), -1)], axis=1)
            ## source 3
            ...
            age_norm = (age - cov_dict["age_min"]) / (cov_dict["age_max"] - cov_dict["age_min"])
            labels2 = np.concatenate([age_norm.reshape(-1, 1), sample_source1[:, 1:],
                                     sample_source2[:, 1:], labels2[:, 1:], 
                                     c_source.reshape(len(labels2), -1)], axis=1)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            ## source 3
            phase_real_img3 = (phase_real_img3.to(device).to(torch.float32) / 127.5 - 1).split(batch_gpu)
            ## merge three sources together (real images)
            phase_real_imgs = phase_real_img1 + phase_real_img2 + phase_real_img3

            ## estimate the latent space (hidden variables)
            if training_set.data_name == "retinal": 
                ## estimate c2, c3, c4
                ## estimate c3, c4 for source 1
                gen_c3 = latent_sampler2.sampling_model(len(phase_real_c1))[-1]
                gen_c4 = latent_sampler3.sampling_model(len(phase_real_c1))[-1]
                c_source = torch.tensor([1, 0, 0] * len(phase_real_c1)).reshape(len(phase_real_c1), -1) ## source 1
                phase_real_c1 = torch.cat([phase_real_c1, gen_c3, gen_c4, c_source], dim=1).to(device).split(batch_gpu)
                ## estimate c1, c2 for source 2
                gen_c12 = latent_sampler1.sample_normalize(len(phase_real_c2))
                gen_c4 = latent_sampler3.sampling_model(len(phase_real_c2))[-1]
                c_source = torch.tensor([0, 1, 0] * len(phase_real_c2)).reshape(len(phase_real_c2), -1) ## source 2
                phase_real_c2 = torch.cat([gen_c12, 
                                        phase_real_c2, gen_c4, c_source], dim=1).to(device).split(batch_gpu)
                ## estimate c1, c2, c3 for source 3
                gen_c12 = latent_sampler1.sample_normalize(len(phase_real_c3))
                gen_c3 = latent_sampler2.sampling_model(len(phase_real_c3))[-1]
                c_source = torch.tensor([0, 0, 1] * len(phase_real_c3)).reshape(len(phase_real_c3), -1) ## source 2
                phase_real_c3 = torch.cat([gen_c12, gen_c3, phase_real_c3, 
                                        c_source], dim=1).to(device).split(batch_gpu)
                ...
                age_norm = (age - cov_dict["age_min"]) / (cov_dict["age_max"] - cov_dict["age_min"])
                phase_real_c3 = torch.cat([age_norm.reshape(-1, 1), gen_c12[:, 1:], 
                                        gen_c3[:, 1:], phase_real_c3[:, 1:], c_source], dim=1).to(device).split(batch_gpu)

            else:
                raise NotImplementedError("Unknown dataset name: ", training_set.data_name)
            ## merge three sources together (real labels)
            phase_real_cs = phase_real_c1 + phase_real_c2 + phase_real_c3

            ### Gen data
            all_gen_z = torch.randn([len(phases) * batch_size * 3, G.z_dim], device=device)
            all_gen_z = [phase_gen_z.split(batch_gpu) for phase_gen_z in all_gen_z.split(batch_size * 3)]
            ## source 1
            ...
            ## source 3
            all_gen_c3 = [training_set2.get_label(np.random.randint(len(training_set2))) for _ in range(len(phases) * batch_size)]

            all_gen_c1 = torch.from_numpy(np.stack(all_gen_c1)) ## (c1), c2
            all_gen_c2 = torch.from_numpy(np.stack(all_gen_c2)) ## (c1), c3
            all_gen_c3 = torch.from_numpy(np.stack(all_gen_c3)) ## (c1), c4

            ## estimate the latent space (hidden variables)
            if training_set.data_name == "retinal": 
                ## estimate (c2, c3, c4)
                ## estimate c3, c4 for source 1
                gen_c3 = latent_sampler2.sampling_model(len(all_gen_c1))[-1]
                gen_c4 = latent_sampler3.sampling_model(len(all_gen_c1))[-1]
                c_source = torch.tensor([1, 0, 0] * len(all_gen_c1)).reshape(len(all_gen_c1), -1) ## source 1
                all_gen_c1 = torch.cat([all_gen_c1, gen_c3, gen_c4, c_source], dim=1).pin_memory().to(device)
                ## estimate c1, c2 for source 2
                gen_c12 = latent_sampler1.sample_normalize(len(all_gen_c2))
                gen_c4 = latent_sampler3.sampling_model(len(all_gen_c2))[-1]
                c_source = torch.tensor([0, 1, 0] * len(all_gen_c2)).reshape(len(all_gen_c2), -1) ## source 2
                all_gen_c2 = torch.cat([gen_c12, 
                                        all_gen_c2, gen_c4, c_source], dim=1).pin_memory().to(device)
                ## estimate c1, c2, c3 for source 3
                gen_c12 = latent_sampler1.sample_normalize(len(all_gen_c3))
                gen_c3 = latent_sampler2.sampling_model(len(all_gen_c3))[-1]
                c_source = torch.tensor([0, 0, 1] * len(all_gen_c3)).reshape(len(all_gen_c3), -1) ## source 3
                all_gen_c3 = torch.cat([gen_c12, gen_c3, all_gen_c3, c_source], dim=1).pin_memory().to(device)
                ...
                all_gen_c1 = torch.cat([age_norm.reshape(-1, 1), all_gen_c1[:, 1:],
                                    gen_c3[:, 1:], gen_c4[:, 1:], c_source], dim=1).pin_memory().to(device)
                ## estimate c1, c2, c4 for source 2
                ...
                c_source = torch.tensor([0, 0, 1] * len(all_gen_c3)).reshape(len(all_gen_c3), -1) ## source 3
                age_norm = (age - cov_dict["age_min"]) / (cov_dict["age_max"] - cov_dict["age_min"])
                all_gen_c3 = torch.cat([age_norm.reshape(-1, 1), gen_c12[:, 1:],
                                        gen_c3[:, 1:], all_gen_c3[:, 1:], c_source], dim=1).pin_memory().to(device)
            else:
                raise NotImplementedError("Unknown dataset name: ", training_set.data_name)
            ...
            all_gen_c = [phase_gen_c.split(batch_gpu) for phase_gen_c in all_gen_c.split(batch_size * 3)]
        ...
        if (snapshot_data is not None) and (len(metrics) > 0):
            ...
            for metric in metrics:
                result_dict = metric_main.calc_metric_tri_ms(metric=metric, G=snapshot_data['G_ema'],
                    dataset_kwargs=validation_set_kwargs, dataset_kwargs_1=validation_set_kwargs1,
                    dataset_kwargs_2=validation_set_kwargs2,
                    sampler = (latent_sampler1, latent_sampler2, latent_sampler3),
