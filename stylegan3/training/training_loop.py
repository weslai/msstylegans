...
def training_loop(
    ...
    val_set_iterator = iter(torch.utils.data.DataLoader(dataset=val_set, sampler=val_set_sampler, batch_size=batch_size//num_gpus, **data_loader_kwargs))
    
    if rank == 0:
        print()
        print("Dataset name: ", training_set.data_name)
        print('Num images train: ', len(training_set))
        print("Num images val: ", len(val_set))
        print('Image shape: ', training_set.image_shape)
        print('Label shape: ', training_set.label_shape)
        print()
        wandb.config.update(
            {
                "dataset": training_set.data_name,
                "num_images": len(training_set),
                "image_shape": training_set.image_shape,
                "label_shape": training_set.label_shape,
                "num_images_val": len(val_set)
            }
        )
