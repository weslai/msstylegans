...
import dnnlib
from training import training_loop_tri_ms
...
def subprocess_fn(rank, c, temp_dir):
    ...
    training_loop_tri_ms.training_loop(rank=rank, **c)
...
@click.option('--data3',         help='Training data 3', metavar='[ZIP|DIR]',                      type=str, required=True)
@click.option('--data_name3',    help='Dataset name 3', metavar='STR',                             type=str, required=True)
...
@click.option('--metrics',      help='Quality metrics', metavar='[NAME|A,B,C|none]',            type=parse_comma_separated_list, default=['fid50k_tri_real'], show_default=True)
@click.option('--kimg',         help='Total training duration', metavar='KIMG',                 type=click.IntRange(min=1), default=14000, show_default=True)
...
def main(**kwargs):
    ...
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss_tri_ms.StyleGAN2Loss')
    c.data_loader_kwargs = dnnlib.EasyDict(pin_memory=True, prefetch_factor=2)

    # Training set.
    if opts.data_scenario == "low" or opts.data_scenario == "lowlow":
        if opts.data_name == "adni" and opts.data_scenario == "low": ## not needed
            max_size = int(7024 / 3)
    ...
    c.validation_set_kwargs1, val_dataset_name2 = init_dataset_kwargs(data=opts.data2, name=opts.data_name2, 
                                                                      ...
                                                                      max_size=None)
    ## third dataset
    c.training_set_kwargs2, dataset_name3 = init_dataset_kwargs(data=opts.data3, name=opts.data_name3, 
                                                                use_labels=opts.cond, mode="train", 
                                                                max_size=max_size2, scenario=opts.data_scenario)
    c.validation_set_kwargs2, val_dataset_name3 = init_dataset_kwargs(data=opts.data3, name=opts.data_name3, 
                                                                      use_labels=opts.cond, mode="val", 
                                                                      max_size=None)                                                                      
    assert val_dataset_name1 == dataset_name1
    assert val_dataset_name2 == dataset_name2
    assert val_dataset_name3 == dataset_name3
    ...
    ## dataset 3
    c.training_set_kwargs2.use_labels = opts.cond
    c.training_set_kwargs2.xflip = opts.mirror
    c.validation_set_kwargs2.use_labels = opts.cond
    c.validation_set_kwargs2.xflip = opts.mirror
