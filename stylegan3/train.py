...
@click.option('--data_name',    help='Dataset name', metavar='STR',                             type=str, required=True)
...
def main(**kwargs):
    ...
    c.validation_set_kwargs, val_dataset_name = init_dataset_kwargs(data=opts.data, name=opts.data_name, use_labels=opts.cond,
                                                                    mode="val",
                                                                    max_size=None)
    assert val_dataset_name == dataset_name
    ...
    # Description string.
    desc = f'{opts.cfg:s}-cond{opts.cond}-{dataset_name:s}-aug{opts.aug:s}-gpus{c.num_gpus:d}-batch{c.batch_size:d}-gamma{c.loss_kwargs.r1_gamma:g}-kimg{c.total_kimg:d}'
