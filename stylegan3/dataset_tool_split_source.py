...
def open_mnist_split(
    ...
    path = os.path.join("/", *json_path.split("/")[:-1])
    if which_dataset == "trainset" or which_dataset == "valset":
        trainset, valset = train_test_split(df, test_size=0.2, random_state=42, shuffle=True)
        trainset.index = range(len(trainset))
        valset.index = range(len(valset))
        max_img = len(trainset) if which_dataset == "trainset" else len(valset)
    else:
        max_img = len(df)
    def iterate_images():
        if which_dataset == "trainset":
            ## trainset
            for idx, row in trainset.iterrows():
                items = row[1]
                imgpath = os.path.join(path, row[0])
                img = np.array(PIL.Image.open(imgpath))
                yield dict(img=img, label=items)
        elif which_dataset == "valset":
...
def open_dataset(source, *, dataset_name: str = None, 
    ...
    if os.path.isdir(source):
        ...
        ## morpho mnist (class-wise fraction/swelling/original)
            ...
            return open_mnist_split(json_path=annotation_path, which_dataset=which_dataset)
