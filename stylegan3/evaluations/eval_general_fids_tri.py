...
from training.dataset_real_ms import AdniMRIDataset2D, KaggleEyepacsDataset, RFMiDDataset, NACCMRIDataset2D
...
@click.option('--data-path2', 'data_path2', type=str, help='Path to the data source 2', default=None, required=True)
@click.option('--data-path3', 'data_path3', type=str, help='Path to the data source 3', default=None, required=True)
...
def run_general_fid(
    ...
    config_dict = {
        ...
        "data_path1": data_path1, "data_path2": data_path2, "data_path3": data_path3,
    ...
    if dataset == "mri":
        ...
        sampler3 = SourceSampling(dataset="nacc", label_path=ds3._path)
        df = sampler1.get_graph()
        df2 = sampler2.get_graph()
        df3 = sampler3.get_graph()
        model = sampler1.get_causal_model()
        model2 = sampler2.get_causal_model()
        model3 = sampler3.get_causal_model()
    ...
    gen_imgs = []
    ## real images
    for source1, source2, source3 in torch.utils.data.DataLoader(concat_ds, batch_size=batch_gen, shuffle=True, **data_loader_kwargs):
        img1, _labels1 = source1[0], source1[1]
        img2, _labels2 = source2[0], source2[1]
        img3, _labels3 = source3[0], source3[1]
    ...
    for _ in range(num_samples // batch_gen):
        batch_labels = []
        z = torch.randn(batch_gen * 3, Gen.z_dim).to(device)
        for _i in range(batch_gen):
            ## (c1, c2, c3)
            source_c1, source_c2, source_c3 = concat_ds[np.random.randint(len(concat_ds))]
            label1, label2, label3 = source_c1[1], source_c2[1], source_c3[1]
            label1_norm = ds1._normalise_labels(label1[0].reshape(-1, 1), label1[1].reshape(-1, 1),
                                                label1[2].reshape(-1, 1)) ## age, vols
            if dataset == "mri":
                age2 = label2[0].reshape(-1, 1) * (ds2.model["age_max"] - ds2.model["age_min"]) + ds2.model["age_min"]
                age3 = label3[0].reshape(-1, 1) * (ds3.model["age_max"] - ds3.model["age_min"]) + ds3.model["age_min"]
                label1_hidd_1 = sampler2.sampling_given_age(label1[0].reshape(-1, 1), normalize=True) ## age, cdr
                label1_hidd_2 = sampler3.sampling_given_age(label1[0].reshape(-1, 1), normalize=True) ## age, apoe, cdr
                label2_hidd_1 = sampler1.sampling_given_age(age2, normalize=True) ## age, vols
                label2_hidd_2 = sampler3.sampling_given_age(age2, normalize=True) ## age, apoe, cdr
                label3_hidd_1 = sampler1.sampling_given_age(age3, normalize=True) ## age, vols
                label1_norm = torch.cat([torch.tensor(label1_norm), label1_hidd_1[0, 1:].reshape(1, -1), 
                                         label1_hidd_2[0, 1:4].reshape(1, -1),
                                         torch.tensor([1, 0, 0]).reshape(1, -1)], dim=1)
                
                label2_norm = torch.cat([label2_hidd_1, torch.tensor(label2[1:].reshape(1, -1)),
                                        label2_hidd_2[:, 1:4].reshape(1, -1),
                                        torch.tensor([0, 1, 0]).reshape(1, -1)], dim=1)
                
                label3_norm = torch.cat([label3_hidd_1, torch.tensor(label3[4:].reshape(1, -1)),
                                        torch.tensor(label3[1:4].reshape(1, -1)),
                                        torch.tensor([0, 0, 1]).reshape(1, -1)], dim=1)
                
            elif dataset == "retinal": ## ukb + eyepacs
                label1_hidd = sampler2.sampling_model(label1[0].shape[0]) ## discease
                label2_hidd = sampler1.sample_normalize(label2[0].shape[0]) ## age, bp, spherical
            ...
            l = torch.cat([label1_norm, label2_norm, label3_norm], dim=0).to(device)
