...
def run_general_mse(opts):
    ...
    num_bins = 3
    ## config
    network_folder = opts.network_folder
    ...
    # Load the network.
    network_names = os.path.join(network_folder, "*.pkl")
    network_list = sorted(glob.glob(network_names))
    ...
    for network_pkl in network_list:
        file_kimg = network_pkl.split("-")[-1].split(".")[0]
        Gen = load_generator(
        ...
        for key, value in covariates_info["cov"].items():
            ...
            scores_df.to_csv(os.path.join(network_folder, f"general_test_loss_{key}_{dataset}_{file_kimg}.csv"), index=False)
            for i in range(len(predictions_dict[key])):
                predictions_dict[key][i].to_csv(os.path.join(network_folder, f"general_test_predictions_{key}_{file_kimg}.csv"),
                                        index=False)

## --------------------------- ##
@click.command()
@click.option('--network_folder', help='Network pickle filefolder', default=None, required=False)
...
def main(**kwargs):
    ...
    assert opts.network_folder is not None
    assert opts.regr_model0 is not None

    
    config_dict = {
        "gen_folder": opts.network_folder,
        "regr_model0": opts.regr_model0,
        "which_model0": opts.which_model0,
        "task0": opts.task0,
        ...
        "num_samples": opts.num_samples}
    with open(os.path.join(opts.network_folder, "strata_mse_config.json"), "w") as f:
